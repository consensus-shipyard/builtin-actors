...
/// Content represented by a `Cid`.
pub trait Content: From<Cid> {
    fn cid(&self) -> Cid;
    fn code(&self) -> Code;
}

pub trait StoreBound<'s, S: 's + Blockstore> {}

/// Operations on content types that, once loaded, are rooted
/// and bound to a block store, and need to be flushed back.
pub trait Stored<'s, S: 's + Blockstore>: Content {
    type Item;
    ...
    fn modify<R>(
        &mut self,
        store: &'s S,
        f: impl FnOnce(&mut Self::Item) -> Result<R>,
    ) -> Result<R> {
...
#[cfg(test)]
mod test {
    use super::{CHamt, CRef, Content, Stored};
    use cid::Cid;
